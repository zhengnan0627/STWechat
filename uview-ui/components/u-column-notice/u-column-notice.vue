<template>
	...
	>
		...
		<swiper :disable-touch="disableTouch" @change="change" :autoplay="autoplay && playState == 'play'" :vertical="vertical" circular :interval="duration" class="u-swiper">
			<swiper-item v-for="(item, index) in list" :key="index" class="u-swiper-item" @tap="click(item)">
				<view
		...
		<view class="u-icon-wrap">
			<text v-if="closeIcon"  @click="close">查看更多</text>
			<u-icon @click="getMore" class="u-right-icon" v-if="moreIcon" name="arrow-right" :size="26" :color="computeColor"></u-icon>
			<u-icon @click="close" class="u-right-icon" v-if="moreIcon"   name="list-dot" :size="24" :color="computeColor"></u-icon>
		</view>
	</view>
</template>
